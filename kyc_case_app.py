...
def _coverage_to_matrix_item(c: dict) -> dict:
    """Reshape one gateway coverage record into the flat shape the Coverage tab renders."""
    df = c.get("dataFields") or {}
    return {
        "code": c.get("code"),
        "name": c.get("name"),
        "group": c.get("group"),
        "sla": c.get("sla"),
        "registryAccess": " / ".join(c.get("registries") or []),
        "companyIdentity": df.get("companyIdentity") or [],
        "controlling": df.get("controllingEntitiesAndIndividuals") or [],
        "shareholders": df.get("shareholdersPartnersAndUBOs") or [],
        "documentsMandatory": [d.get("description") for d in (c.get("baseDocuments") or [])],
        "documentsNonMandatory": [d.get("description") for d in (c.get("additionalDocuments") or [])],
    }
    ...
    def do_GET(self):
        ...
        try:
            if u.path in ("/", "/index.html"):
                ...
                items = [_coverage_to_matrix_item(c) for c in gw.list_coverage()]
                _json(self, {"jurisdictions": items})
            elif u.path == "/api/jurisdiction":
                code = (q.get("code", [None])[0] or "").strip() or None
                if not code:
                    return _json(self, {"error": "code required"}, 400)
                try:
                    _json(self, gw.coverage(code))
                except requests.HTTPError as e:
                    st = e.response.status_code if e.response is not None else 502
                    return _json(self, {"error": _upstream_error(e)}, 404 if st == 404 else 502)
                ...
                slow_msg = ("This registry is taking too long to respond (some jurisdictions, "
                            "e.g. Cayman, are slow). Please refine your search or try again in a moment.")
                try:
                    results = gw.search(jurisdiction=jur, query=query, datasource=ds)
                except requests.HTTPError as e:
                    st = e.response.status_code if e.response is not None else 400
                    if st == 504:
                        # The gateway bounds slow registries and returns a clean 504.
                        return _json(self, {"error": slow_msg, "slow": True,
                                            "jurisdiction": jur, "query": query}, 504)
                    # The registry's own message (e.g. "More than 200 records found … refine")
                    # is passed through by the gateway as a 4xx — surface it as-is.
                    ...
                    return _json(self, {"error": slow_msg, "slow": True,
                                        "jurisdiction": jur, "query": query}, 504)
                # The gateway attaches a uniform `enrichment` object to every hit already.
